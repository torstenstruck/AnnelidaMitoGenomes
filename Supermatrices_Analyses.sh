...
echo "cp ${FILE} ${FILE}_ConstraintTree.tre ${FILE}_partition.txt \$workdir" >> iqtree_${FILE}.sh
echo 'cd $workdir' >> iqtree_${FILE}.sh
echo "" >> iqtree_${FILE}.sh

echo "## Generate the phylogenetic tree of the aligned sequences:" >> iqtree_${FILE}.sh
echo "iqtree -s ${FILE} -m MFP+MERGE -g ${FILE}_ConstraintTree.tre -spp ${FILE}_partition.txt -pre ${FILE} -nt AUTO" >> iqtree_${FILE}.sh
echo "" >> iqtree_${FILE}.sh
